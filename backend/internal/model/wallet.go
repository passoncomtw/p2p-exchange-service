...
// FreezeInTx deducts amount from available_balance and adds to frozen_balance within a caller-provided session.
func (m *WalletModel) FreezeInTx(ctx context.Context, session sqlx.Session, userID int64, currency, amountStr string) error {
	var wallet Wallet
	if err := session.QueryRowCtx(ctx, &wallet,
		`SELECT id, user_id, currency, available_balance::text, frozen_balance::text, created_at, updated_at
		 FROM wallets WHERE user_id = $1 AND currency = $2 FOR UPDATE`,
	...
	avail, _, _ := new(big.Float).Parse(wallet.AvailableBalance, 10)
	need, _, _ := new(big.Float).Parse(amountStr, 10)
	if avail.Cmp(need) < 0 {
		return apperrors.New(400, "insufficient balance")
	}
...
// DepositWithLedgerType credits amount to user's available_balance with a custom ledger type.
// Used for crypto_deposit and fiat_deposit flows.
func (m *WalletModel) DepositWithLedgerType(ctx context.Context, userID int64, currency, amount, ledgerType string) error {
	if m.rdb != nil {
		unlock, err := m.rdb.AcquireLock(ctx, WalletLockKey(userID, currency), walletLockTTL)
		if err != nil {
			return err
	...
	return m.conn.TransactCtx(ctx, func(ctx context.Context, session sqlx.Session) error {
		...
		_, err = session.ExecCtx(ctx,
			`INSERT INTO wallet_ledgers (wallet_id, type, amount, balance_after)
			 VALUES ($1, $2, $3, $4)`,
			wallet.ID, ledgerType, amount, wallet.AvailableBalance,
		)
		return err
	})
}

// DeductFrozenBalance moves amount out of frozen_balance (withdrawal confirmed).
// Adds a ledger entry of the given type (e.g. "crypto_withdraw").
func (m *WalletModel) DeductFrozenBalance(ctx context.Context, userID int64, currency, amount, ledgerType string) error {
	if m.rdb != nil {
		unlock, err := m.rdb.AcquireLock(ctx, WalletLockKey(userID, currency), walletLockTTL)
		if err != nil {
			return err
	...
	return m.conn.TransactCtx(ctx, func(ctx context.Context, session sqlx.Session) error {
		...
		_, err := session.ExecCtx(ctx,
			`INSERT INTO wallet_ledgers (wallet_id, type, amount, balance_after)
			 VALUES ($1, $2, $3, $4)`,
			wallet.ID, ledgerType, negAmount, newFrozen,
		)
		return err
	})
}

// UnfreezeBalance returns amount from frozen_balance back to available_balance (withdrawal failed).
func (m *WalletModel) UnfreezeBalance(ctx context.Context, userID int64, currency, amount string) error {
	if m.rdb != nil {
		unlock, err := m.rdb.AcquireLock(ctx, WalletLockKey(userID, currency), walletLockTTL)
		if err != nil {
			return err
