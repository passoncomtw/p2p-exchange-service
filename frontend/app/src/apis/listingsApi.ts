...
export const listingsApi = {
  list: async (params?: ListListingsParams): Promise<ListingItem[]> => {
    const response = await httpClientWithAuth.getWithToken<ApiResponse<ListListingsResponse>>(
      '/app/listings',
      { params }
    );
  ...
  getById: async (id: number): Promise<ListingItem> => {
    const response = await httpClientWithAuth.getWithToken<ApiResponse<ListingItem>>(
      `/app/listings/${id}`
    );
