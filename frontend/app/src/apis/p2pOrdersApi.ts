...
export const p2pOrdersApi = {
  list: async (params?: ListOrdersParams): Promise<Order[]> => {
    const response = await httpClientWithAuth.getWithToken<ApiResponse<ListOrdersResponse>>(
      '/app/orders',
      { params }
    );
  ...
  getById: async (id: number): Promise<Order> => {
    const response = await httpClientWithAuth.getWithToken<ApiResponse<Order>>(
      `/app/orders/${id}`
    );
  ...
  create: async (params: CreateOrderParams): Promise<{ id: number; orderNo: string }> => {
    const response = await httpClientWithAuth.postWithToken<ApiResponse<{ id: number; orderNo: string }>>(
      '/app/orders',
      params
    );
    return response.data.data;
